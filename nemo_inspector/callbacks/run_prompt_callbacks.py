    ...
    if examples_type == RETRIEVAL:
        ...
        if (
            ...
            if retrieval_field_index != -1:
                retrieval_field = raw_utils[retrieval_field_index]["props"]["children"][
                    1
                ]["props"]
                retrieval_field_value = raw_utils[retrieval_field_index]["props"][
                    "children"
                ][1]["props"]["value"]
...
@app.callback(
    ...
    [
        Input("prompt_config_path", "value"),
        Input("template_path", "value"),
    ],
    State("js_trigger", "children"),
    prevent_initial_call=True,
)
def update_prompt_type(
    config_path: str, template_path: str, js_trigger: str
) -> Union[NoUpdate, dbc.AccordionItem]:
    if (
        "used_prompt" in current_app.config["nemo_inspector"]["prompt"]
        and (config_path, template_path)
    ...
    current_app.config["nemo_inspector"]["prompt"][
        "stop_phrases"
    ] = prompt_config_path.template.stop_phrases

    return [
        get_utils_field_representation(value, key)
        for key, value in get_utils_from_config(asdict(prompt_config_path)).items()
    ] + ["", js_trigger + " "]
